...
# Use parallel workers for faster processing (recommended)
# Check your CPU cores first
sysctl -n hw.ncpu  # macOS
nproc              # Linux

# Use 50-70% of your cores for workers (e.g., 6 workers for 10-core M4)
marker_single file.pdf --output_dir . --pdftext_workers 6

# Process in background (for large files)
marker_single large_file.pdf --output_dir . --pdftext_workers 6 > conversion.log 2>&1 &
...
```bash
# Check CPU cores to determine worker count
sysctl -n hw.ncpu  # Returns 10 for M4 Mac

# Convert the PDF with parallel workers (6 workers for 10-core CPU)
marker_single WHR25.pdf --output_dir . --pdftext_workers 6

# Or run in background and monitor
marker_single WHR25.pdf --output_dir . --pdftext_workers 6 > conversion.log 2>&1 &
tail -f conversion.log
...
For a **263-page PDF** (World Happiness Report 2025):
- **Model download** (first time only): ~5-10 minutes (depending on internet speed)
- **PDF processing**:
  - Without workers: ~40-60 minutes
  - With workers (6 workers on 10-core M4): ~20 minutes
  - Layout detection
  - Text recognition
  - Table recognition
  - OCR error detection
...
**Tip:** Using `--pdftext_workers` can reduce processing time by 50% or more on multi-core systems.
...
```bash
#!/bin/bash
# batch_convert.sh - Convert multiple PDFs with parallel workers

# Determine worker count (50-70% of CPU cores)
CORES=$(sysctl -n hw.ncpu 2>/dev/null || nproc)
WORKERS=$(( CORES * 6 / 10 ))

echo "Using $WORKERS workers on $CORES cores"

for pdf in *.pdf; do
    echo "Converting $pdf..."
    base=$(basename "$pdf" .pdf)
    marker_single "$pdf" --output_dir "${base}_converted" --pdftext_workers "$WORKERS"
    echo "✓ Completed $pdf"
done
...
### Input
- PDF: 263 pages, multi-column layout, 159 images
- Size: 22 MB
- URL: https://files.worldhappiness.report/WHR25.pdf

### Output
- Markdown: 653 KB, properly formatted
- Images: 159 JPEG files (all figures, charts, and diagrams)
- Quality: Excellent - no column-merge issues

### Processing Stats (M4 Mac, 10 cores)
- Model download: ~5 minutes (first time only)
- Conversion time: ~19.5 minutes (with 6 workers)
...
# Check CPU cores
sysctl -n hw.ncpu  # Returns 10

# Convert to markdown with parallel workers
marker_single WHR25.pdf --output_dir . --pdftext_workers 6

# Result: WHR25/WHR25.md + 159 image files
```
